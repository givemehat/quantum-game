...
app.layout = html.Div(
    [
        ...
        html.Div(
            [
                html.H1(
                    "Quantum-Chess HFT Framework",
                    style={"color": COLORS["primary"], "marginBottom": "5px"},
                ),
                html.P(
                    "Research-Grade Trading Intelligence System",
            ...
            [
                ...
                html.Div(
                    [
                        ...
                        dcc.Graph(
                            id="market-heatmap", config={"displayModeBar": False}
                        ),
                        html.H3(
                ...
                html.Div(
                    [
                        ...
                        dcc.Graph(
                            id="strategy-evolution", config={"displayModeBar": False}
                        ),
                        html.H3(
                            "Latency Profile",
                            style={"color": COLORS["text"], "marginTop": "20px"},
                        ...
                        dcc.Graph(
                            id="latency-profile", config={"displayModeBar": False}
                        ),
                    ],
                    style={
                        "width": "48%",
...
@app.callback(
    Output("decision-tree", "figure"), [Input("interval-component", "n_intervals")]
)
...
@app.callback(
    Output("action-probs", "figure"), [Input("interval-component", "n_intervals")]
)
def update_action_probabilities(n_intervals):
    ...
    colors = [
        COLORS["ask"] if i < 5 else COLORS["primary"] if i > 5 else COLORS["secondary"]
        for i in range(11)
    ]
...
@app.callback(Output("risk-surface", "figure"), [Input("update-button", "n_clicks")])
...
@app.callback(
    Output("strategy-evolution", "figure"), [Input("interval-component", "n_intervals")]
)
def update_strategy_evolution(n_intervals):
    ...
    sharpe_qc = (
        0.8 + 0.25 * np.log(episodes + 1) + np.random.randn(len(episodes)) * 0.05
    )
...
@app.callback(Output("latency-profile", "figure"), [Input("update-button", "n_clicks")])
