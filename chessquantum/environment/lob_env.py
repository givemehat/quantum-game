...
class Order:
    ...
    def __init__(self, order_id: int, price: float, volume: int, side: str, timestamp: float):
        ...
        if side == 'sell':  # Closing long positions
            while remaining > 0 and self.long_positions:
                ...
                if close_volume == buy_volume:
                    ...
                    self.long_positions[0] = (buy_price, buy_volume - close_volume, buy_time)
            ...
            while remaining > 0 and self.short_positions:
                ...
                if close_volume == sell_volume:
                    ...
                    self.short_positions[0] = (sell_price, sell_volume - close_volume, sell_time)
...
class LimitOrderBookEnv(gym.Env):
    ...
    def __init__(self, data_path: str = None, window_size: int = 100,
                 max_position: int = 1000, transaction_cost: float = 0.0001):
        """
    ...
    def _execute_market_order(self, side: str, volume: int, price: float):
        ...
        price_with_cost = price * (1 + self.transaction_cost if side == 'buy' else 1 - self.transaction_cost)
    ...
    def _compute_reward(self) -> float:
        ...
        pnl_change = self.ledger.realized_pnl - self.prev_pnl if hasattr(self, 'prev_pnl') else 0
