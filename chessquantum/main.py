...
class QuantumChessHFT:
    ...
    def __init__(self, config: Dict):
        ...
        self.quantum_layer = TradingQAOA(
            n_qubits=config.get("n_qubits", 12), p_layers=config.get("p_layers", 2)
        )
    ...
    def select_action_hybrid(self, state: np.ndarray) -> int:
        ...
        quantum_probs = self.quantum_layer.get_action_probabilities(
            market_state=state.flatten(), action_rewards=action_rewards
        )

        # Step 2: RL agent - get policy probabilities
        rl_action, rl_log_prob, rl_value = self.rl_agent.select_action(
        ...
        best_action, search_stats = self.search_engine.get_best_action(
            state.flatten(), inventory=self.env.ledger.inventory
        )
    ...
    def _estimate_action_reward(self, state: np.ndarray, action: int) -> float:
        ...
        if len(state) > 0:
            ...
            spread = (
                ...
                else state[1] if len(state) > 1 else 0.01
            )
            imbalance = (
                state[0, 4]
                if len(state.shape) > 1
                else state[4] if len(state) > 4 else 0.0
            )
    ...
    def run_backtest(self, n_steps: int = 1000) -> Dict:
        ...
        for step in range(n_steps):
            ...
            if step % 100 == 0 and step > 0:
                ...
                print(
                    f"Step {step}: PnL={info['realized_pnl']:.2f}, "
        ...
        metrics = {
            ...
            "max_inventory": (
                max(self.inventory_history) if self.inventory_history else 0
            ),
